...
async function main() {
  ...
  // create transaction
  const safeSdk_account_1 = await createSafeSdk(ethAdapter_account_1, BSCT_SAFE_ADDRESS, isEthereum)
  ...
  // approve transaction
  const safeSdk_account_2 = await createSafeSdk(ethAdapter_account_2, BSCT_SAFE_ADDRESS, isEthereum)
